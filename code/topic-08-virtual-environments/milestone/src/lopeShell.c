/*
 * lopeShell.c — CST-315 · Topic 8 · milestone (final)
 * Purpose: Final cumulative shell. Identical to the Topic 7 version
 *          (Topics 2-3 features plus argv sanitization). This is the
 *          binary Project 7 demos.
 * Build:   make
 * Run:     ./lopeShell
 * Notes:   POSIX. Builds on macOS and Linux. Also builds inside the
...
static int sanitize_argv(int argc, char *argv[MAX_ARGS]) {
    for (int i = 0; i < argc; i++) {
        if (!safe_arg(argv[i])) {
            fprintf(stderr, "lopeShell: rejected argv[%d] '%s' (metacharacter)\n", i, argv[i]);
            return 0;
        }
    }
    return 1;
