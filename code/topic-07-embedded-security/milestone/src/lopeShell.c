/*
 * lopeShell.c — CST-315 · Topic 7 · milestone (v3)
 * Purpose: Topic 3 shell with an argv-sanitization pass. Any token
 *          containing a shell metacharacter (; & | $ ` < > newline
 *          single-quote double-quote) is rejected before fork.
 * Build:   make
 * Run:     ./lopeShell
 * Notes:   POSIX. Builds on macOS and Linux. Cumulative on Topics
 *          2 and 3; the only new code is sanitize() and its caller.
 */
...
/* v3: sanitize one argv entry. Return 1 if safe, 0 if it contains
 * a shell metacharacter that has no business in a tokenized argv. */
static int safe_arg(const char *s) {
    static const char banned[] = ";&|$`<>\n'\"";
    return strpbrk(s, banned) == NULL;
}
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        for (int i = 0; i < ncmds; i++) {
            ...
            /* v3: sanitization pass — reject anything suspicious. */
